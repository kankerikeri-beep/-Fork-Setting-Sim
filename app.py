...
default_params = {
    ...
    "m_bike": 90.0, "m_rider": 68.0, "caster_angle": 25.0, "decel_g": 1.25,
...
st.info("このセクションで入力した情報とシミュレーターの数値を合体させ、AIへ渡す完璧な指示書を自動生成します。")
...
with col_out2:
    st.write("**STEP 2: 燃調マップ等の準備（任意）**")
    st.caption("セッティングツール等の「燃調マップ画面のスクショ」があれば用意してください。AIから具体的な変更数値の提案が可能になります。")
