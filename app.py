...
"""
                ...
                focus_instruction += "・【★動画解析の統合（必須）】動画ファイルが添付されている場合は、映像から確認できる「走行ライン（V字/U字等）」「前走者とのアプローチの違い」「スロットル/ブレーキのタイミング」などの視覚情報を必ず抽出し、CSVのストロークやGの数値と関連付けて解説してください。\n"
...
1. [データ抽出とコース把握]
                   ...
                   ・【🎥 映像からの視覚的状況】(※動画添付時のみ出力) 動画から読み取れる走行ライン、アプローチや旋回操舵の違い、前走者との位置関係、の特徴を具体的に記載。
                2. [旋回を引き出す前後動作]
