...
"""
                ...
                ・【★G数値・ストロークの具体的引用】分析を記述する際は、必ずデータ要約から抽出されたGの数値やストローク量(mm)などの具体的な数値を引用し、シビアで論理的な分析を行ってください。
                ...
                【重要】抽出されたGの数値やストローク量(mm)を具体的に引用し、以下の4点から動的な姿勢変化をシビアに評価すること。
                ...
                1. [データ抽出とコース把握]
                   ...
                   ・【最大ストロークと安定平均位置】前後サスの最大ストローク位置と安定平均の数値を記載すること。
                2. [旋回を引き出す前後動作]
                   ・ブレーキング分析: (評価結果を数値を用いて記載)
                   ・倒しこみ動作: (評価結果を数値を用いて記載)
                   ・初期旋回操舵: (評価結果を数値を用いて記載)
                   ・後期旋回操舵: (評価結果を数値を用いて記載)
                   ・走行ライン: (※【走行ライン比較】の区間タイム・距離・ボトム速度を基に、立ち上がり重視かコーナリング重視かを層別し、ライン違いによるロスと可能性を解析。違いが少ない場合は他の周回との比較を記載。路面リスクがあれば併記すること)
                3. [加速状態での旋回操舵]
                   (トラクションと車体の乱れに関する評価結果を数値を用いて記載)
                4. [燃調の影響と総合解決策]
                   (燃調の確認結果と修正案を記載)
