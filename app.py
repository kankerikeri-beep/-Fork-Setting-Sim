...
with col_ai2:
    ...
    st.info("💡 **【AIの精度を劇的に上げるコツ】**\nAIはデータから自動で現在地を特定できません。「KTCコーナー」といった名称だけでなく、**「コントロールライン通過後、約25秒付近のブレーキング」**のように**経過時間（秒数）**を添えていただくと、AIが的確なポイントのGやストロークを解析できます。")
    
    user_comment = st.text_area("具体的な悩み・知りたいこと（★良いと感じている部分もあれば記載）", placeholder="例：コントロールライン通過後から約25秒後、S字進入の切り返しでフロントが戻ってこない感覚がある。", height=150, key="user_comment")
# ===============================
# ★ AI連携用ファイルアップロードと実行
# ===============================
st.write("---")
...
with col_out2:
    ...
    # ★復活：燃調マップ等の画像アップロード
    # ▼「コース図」という言葉を追加
    uploaded_images = st.file_uploader("コース図・燃調マップ・設定画面のスクショ等があればアップロード（任意）", type=["png", "jpg", "jpeg"], accept_multiple_files=True)
...
if st.button("AIに事前処理（ADA）をかけて解析させる", type="primary"):
    if not uploaded_logs:
        ...
        with st.spinner("データエンジニア(Python)が小区間・中区間のタイム算出と高度な解析を実行中..."):
            try:
                            ...
                            if 'Lap' in log_df.columns and len(valid_laps) > 0:
                                ...
                                if not best_lap_df.empty:
                                    ...
                                    if g_col_to_use:
                                        ...
                                        # --- ★新機能：小・中区間を含む走行ライン評価 ---
                                        ada_summary.append("\n▼ 【走行ライン比較】トップ2箇所の小・中区間タイム（トータルロス評価用）")
                                        for sec in top_braking_sorted:
                                            pct = (sec['rel_time'] / best_lap_df['RelTime'].max()) * 100
                                            sec_str = f" [減速 {sec['rank']}位アプローチ] (Lap進行度 約{int(pct)}%地点からの区間):\n"
                                            for lap_n in target_laps:
                                                l_df = log_df[log_df['Lap'] == lap_n]
                                                l_start = l_df['RunTime'].min()
                                                ...
                                                if not search_start.empty and not search_end.empty and 'Speed' in l_df.columns:
                                                    ...
                                                    # 加速開始点（小区間の終わり）を特定（0.1G以上）
                                                    a_ends = search_end[search_end[g_col_to_use] > 0.1]
                                                    t_end = a_ends['RunTime'].min() if not a_ends.empty else peak_time_abs + 3.0
                                                    
                                                    # 次のコーナーの進入（中区間の終わり）を特定（加速開始から最低2秒後以降で再び-0.2G未満）
                                                    next_brake_search = l_df[(l_df['RunTime'] > t_end + 2.0)]
                                                    if not next_brake_search.empty:
                                                        next_b_starts = next_brake_search[next_brake_search[g_col_to_use] < -0.2]
                                                        t_next_brake = next_b_starts['RunTime'].min() if not next_b_starts.empty else l_df['RunTime'].max()
                                                    else:
                                                        t_next_brake = l_df['RunTime'].max()

                                                    # 小区間データ集計 (旋回のみ)
                                                    small_df = l_df[(l_df['RunTime'] >= t_start) & (l_df['RunTime'] <= t_end)]
                                                    small_time = t_end - t_start
                                                    small_dist = (small_df['Speed'] / 3.6 * small_df['dt']).sum() if not small_df.empty else 0
                                                    min_speed = small_df['Speed'].min() if not small_df.empty else 0
                                                    
                                                    # 中区間データ集計 (旋回＋次コーナーまでのストレート)
                                                    med_df = l_df[(l_df['RunTime'] >= t_start) & (l_df['RunTime'] <= t_next_brake)]
                                                    med_time = t_next_brake - t_start
                                                    med_dist = (med_df['Speed'] / 3.6 * med_df['dt']).sum() if not med_df.empty else 0

                                                    mark = "★ベスト" if lap_n == best_lap_num else "比較"
                                                    sec_str += f"   - Lap {int(lap_n)} ({mark}):\n"
                                                    sec_str += f"      [小区間(旋回)]: ﾀｲﾑ {small_time:.2f}秒 / 距離 {small_dist:.1f}m / ﾎﾞﾄﾑ {min_speed:.1f}km/h\n"
                                                    sec_str += f"      [中区間(次ｺｰﾅｰ迄)]: ﾀｲﾑ {med_time:.2f}秒 / 距離 {med_dist:.1f}m\n"
                                            ada_summary.append(sec_str)
...
"""
                ...
                ・【★AIフリーズ対策（推測指示）】バンク角、前後ブレーキ圧、AFR(燃調)などの対象センサーデータがCSVに存在しない場合は、「データがありません」と放棄せず、G推移やストローク量、車速等から挙動を推測して論理的に回答すること。
                ・【★反力テーブルの参照】エアバネ反力について言及する際は、添付された【反力テーブル(CSV)】の反力荷重(N)の立ち上がり数値を参照し、どの領域を使用しているか検証すること。
                """
...
　　　　　　　　　　　focus_instruction += """
                ・【★コーナー名称の幻覚（ハルシネーション）の完全禁止】
                AIはGPS座標を持たないため、CSVの走行時間のみから実際のサーキットの固有コーナー名を絶対に推測・断定しないでください。
                ・【★「ユーザー指定時間」の最優先と、第1コーナー固定観念の完全排除（重要）】
                ユーザーが「悩み」の中で特定の名称（例：「KTCコーナー」）と時間指定（例：33秒後）を併記した場合、その名称はその秒数専用のラベルと見なしてください。
                AI独自の判断で、その名称を**「コントロールライン通過直後の最初の減速ポイント（いわゆる1コーナー、Lap進行度5〜10%付近）」に勝手に適用して分析することを厳禁します。**（今回のエラー原因：33秒後を指定されているのに、AIが最初の減速（約3〜5秒後、Lap進行度8%）をKTCコーナーと誤断定した）。
                いかなる理由があろうとも、ユーザー指定のタイムスタンプ（〇〇秒後）を優先し、AIの「第1コーナー」固定観念を捨ててください。今回の解析では33秒付近に「KTCコーナー」というラベルを貼り、そこを集中的に解析すること。
                ・【★時間指定とコース図のリンク】
                ユーザーからの「悩み」の中に「コントロールライン通過後〇秒」といった時間指定がある場合、必ずCSVデータ内のその該当秒数付近のG推移やストローク波形を特定して解析の軸にしてください。また、コース図の画像が添付されている場合は、その秒数の挙動（減速か、右旋回か等）とコース図のレイアウトを照らし合わせ、「〇秒付近の挙動は、コース図における『〇〇コーナー』のアプローチに該当すると推測されます」といった形で、論理的に名称をリンクさせることは許可します。
                ・【★動画解析の絶対ルール（表面的な描写の禁止）】
                動画が添付されている場合、「バイクが左に曲がっています」といった単なる状況描写は絶対に禁止します。以下の2点のみを視覚情報から相対評価し、必ずCSVのロガー数値（G、ストローク、速度）を根拠として結びつけて解説してください。
                1. [前走者との相対距離の推移]: ブレーキング開始〜クリッピング〜立ち上がりの各フェーズで、前走者との距離が「どう変化したか」を視覚的に評価し、その原因が自身の減速Gの立ち上がりの遅さなのか、ボトムスピードの差なのかをCSVデータから特定すること。
                2. [倒し込み（ロール）スピードと旋回操舵の比較]: 自身のバイクがバンクしていくスピード（倒し込みの鋭さ）と前走者を比較評価し、その瞬間のフロントフォークのストローク使用率（%）や減速Gの変動とリンクさせて解析すること。
                """
                ...
                ■ [旋回を引き出す前後動作] の評価基準
                サスペンションの挙動が「鋭い旋回性を引き出すための動的な姿勢変化による積極的な荷重付与」を作り出せているかを評価の軸として以下の4点で評価する、また下記4項目でのフロントとリアのストロークの平均位置、バンク角を可能であれば明示する。
                ・ブレーキング: フロントの高い初期ストロークスピードと適切なリアのリフト状態を作り出せているか。タイヤグリップの限界に合わせたフロントブレーキリリースができているか。また逆に倒しこみで発生する遠心力でのリア荷重を積極的に利用したリアブレーキでの減速を実現できているか。減速の最大Gを表記してブレーキング技術の基準として扱うが、全体にスムーズな操作で実現するリアストロークを利用しないR半径の大きい走行も高いレベルの場合は否定しないこと、エアバネ反力のグラフスロープのどのような部位を使っているかも指摘。
                ・倒しこみ: ブレーキ終盤からのフロント側操舵(前後ブレーキ、バンク角、舵角補助)の状態を倒しこみで作り出せているか、また初期後期旋回(特にリアステアの後期旋回)へと影響する倒しこみの早さから付加される荷重を利用したサスペンションのストロークコントロールが出来ているか。
                ・初期旋回: 初期旋回減速と加速の間の旋回瞬間のフルバンク状態で、適切なフロントブレーキのリリースコントロールによりタイヤつぶしすぎない荷重を与え、適切なフロントストロークによって生まれる瞬間的で強烈なフロントステアによる実舵角をより引き出せる操舵状態を作り出せているか。またフロントステアを邪魔しない瞬間的なリアブレーキのリリースコントロール(解放ではない)でリアへの遠心力荷重の付加と後期旋回へのリア荷重準備状態を作り出せているか。
                ・後期旋回: 後期旋回はエンジンブレーキによるマイナス駆動力がフラットからプラス駆動力へ移行する瞬間、前後ブレーキ操作(特にリアブレーキ)と荷重コントロールでリアタイヤを積極的に潰し変形させ、リアが外側軌道へと向かわせるリアステア状態を作り出せているか、リアブレーキの引きづりを利用しリアストロークを積極的にコントロールしたアクセルオープンが出来ているか。
                ※注意：切り返し（S字）などの連続コーナーを評価する場合、手前のコーナーの「脱出動作」なのか、本題のコーナーへの「進入動作」なのかを、時系列データから明確に区別して記載する。

                ■ 【★走行ラインとアプローチの評価基準】
                ブレーキングアプローチの違い、加速状態の違いを以下の3種に層別して評価すること。コーナーのアプローチ開始から次のコーナーまでのアプローチ開始までを中区間タイムとして抽出し評価に用いる。
                1. 立ち上がり重視：進入から比較的早く瞬間的な初期後期旋回操舵でバイクが比較的早く起きて負荷のない加速状態を作り出す走法。（直線的なブレーキングで早めにレ字ライン）
                2. コーナリング重視：進入より高いスピードを深いバンク角を維持しエッジグリップを最大限利用する走法。（走行距離が長く、ボトム速度が高いU字ライン）
                3. ブレーキング進入重視 : クリッピングポイントをコーナーのエイペックスに近い位置に取り、最短距離で若干旋回させながらでもブレーキングさせる走法（走行距離が一番短くボトム速度も一番低いV字ライン）
                データ要約の【走行ライン比較（小区間・中区間）】のタイムと走行距離の違いからトータルロスを評価し、現状の報告と可能性の提示を行うこと。また、路面状態の悪さやミュー（摩擦係数）の低さがストロークデータ（小刻みな振動や抜け等）に現れている場合は、リスクとして報告すること。
                ...
                ■ [燃調・その他の補足基準]
                ・燃調: AfrやRpmからトルク変動の悪影響がないか確認し、必要に応じた燃調修正案を提示すること。O2センサの分解能で確認できない可能性有れば、加速ポンプ調整できないECUに対して疑似加速ポンプ的なパーシャル領域と全開領域の中間部分を濃くするような方策も提案すること。
                ...
                ・フロントストローク奥の部位を使用している解析の場面ではエアバネ反力は適切な部位を使えているかグラフを見返せるようなストローク数値を提示し、検証することを促す事。
                ・解析ラップの絞り込みを行う場合でも条件以外のラップで指摘すべき挙動があった場合は記述し引き合いに出すなどする、例えば残ストロークを表現する場合でベストラップで余裕があったとしてもその他のラップで余裕がないような場面が見受けられる場合は、走らせ方等でごまかしている可能性がある等分析を行う
                =========================================
...
1. [データ抽出とコース把握]
                   ・【コーナー名称のシンプルな明示】「Lap進行度 O%」という表記に、AIの推測したシンプルな名称を割り当てて記述。
                   ・【★ブレーキング減速G トップ2箇所】データ要約のリストを引用。
                   ・【★ストレート加速G（最高速度併記）】データ要約のリストを引用。
                   ・【最大ストロークと安定平均位置】前後サスの最大ストローク位置と安定平均の数値を記載(※ここは絶対値mm表記)。
                   ・【🎥 前走者とのギャップ推移とアプローチ比較】(※動画添付時のみ出力) 単なる状況描写は不可。ブレーキング〜旋回〜立ち上がりにおける前走者との「相対的な距離の増減」と「倒し込みの鋭さの違い」を評価し、なぜその差が生まれたのかをCSVの「G・速度・ストロークの数値」を用いて論理的に解説すること。
                2. [旋回を引き出す前後動作]
                   (※以下の各項目では、必ずストローク%・残ストロークmm・Gの数値を引用し、データがあればバンク角も明記すること。動画添付時は、映像から見えるフォームやライン取り等の視覚情報と数値データの整合性も分析に含めること)
                   ・ブレーキング分析: (評価結果を記載。エアバネ反力テーブルの数値を参照し、どの反力領域を使っているか検証)
                   ・倒しこみ動作: (評価結果を記載。フロント操舵と荷重コントロールに言及)
                   ・初期旋回操舵: (評価結果を記載。フロント/リアブレーキのリリースコントロールによる姿勢変化に言及)
                   ・後期旋回操舵: (評価結果を記載。リアステア状態とリアブレーキの引きずり・変形に言及)
                   ・走行ラインとアプローチ: (※【走行ライン比較】の小・中区間タイム・距離・ボトム速度を基に、[1.立ち上がり重視 / 2.コーナリング重視 / 3.ブレーキング進入重視] の3種のどれに該当するかを層別し、ライン違いによるロスと可能性を解析。路面リスクがあれば併記)
                3. [加速状態での旋回操舵]
                   (トラクション抜け、車体の乱れ[跳ね/チャタリング]に関する評価結果を記載。TCやステダンの介入も推測)
                4. [燃調とその他の車体影響]
                   ・燃調: (トルク変動を分析し、パーシャル領域の疑似加速ポンプ的な修正案等も提示)
                   ・実舵角とポジション: (バンク角との相関、およびハンドル/シート/ステップ等ライディングポジション起因の姿勢変化に言及)
                5. [課題解決のための操舵技術とサスセッティング案]
                   (悩み「{user_comment}」を解決するためのセッティング案と、リアブレーキ/ストロークの積極的な活用方法を提示。バネのセット方向[密巻き/荒巻]も考慮すること)
