...
"""
                ...
                AI独自の判断で、その名称を「コントロールライン通過直後の最初の減速ポイント（いわゆる1コーナー、Lap進行度5〜10%付近）」に勝手に適用して分析することを厳禁します。（今回のエラー原因：33秒後を指定されているのに、AIが最初の減速をKTCコーナーと誤断定した）。
                いかなる理由があろうとも、ユーザー指定のタイムスタンプ（〇〇秒後）を優先し、AIの「第1コーナー」固定観念を捨ててください。今回の解析では指定された秒数付近に「ユーザー指定コーナー」というラベルを貼り、そこを集中的に解析すること。
                ・【★時間指定とコース図のリンク】
