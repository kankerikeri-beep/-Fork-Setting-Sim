...
focus_instruction = f"""
                ...
                1. [データ抽出とコース把握]
                   ...
                   ・【★ブレーキングG トップ3箇所（コーナー別・ラップ別）】コース上で減速Gが大きい上位3箇所のブレーキングポイント（推測したコーナー名）を特定し、各ポイントについて「ベストラップの数値」と「他の2ラップの数値」の計3ラップ分（3箇所×3ラップ＝計9データ程度）を並べてリストアップすること。これにより、特定のラップだけに突出した異常値（ノイズ）が出ていないかをユーザーが比較確認できるようにすること。
