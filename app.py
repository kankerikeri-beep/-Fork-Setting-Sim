...
動画が添付されている場合、「バイクが左に曲がっています」といった単なる状況描写は絶対にしないでください。以下の2点のみを視覚情報から相対評価し、必ずCSVのロガー数値（G、ストローク、速度）を根拠として結びつけて解説してください。
1. [前走者との相対距離の推移]: ブレーキング開始〜クリッピング〜立ち上がりの各フェーズで、前走者との距離が「どう変化したか（急激に詰まった、クリップで離された等）」を視覚的に評価し、その原因が自身の減速Gの立ち上がりの遅さなのか、ボトムスピードの差なのかをCSVデータから論理的に特定すること。
2. [倒し込み（ロール）スピードと旋回操舵の比較]: 自身のバイクがバンクしていくスピード（倒し込みの鋭さ）と、前走者のアプローチを比較評価すること。また、その倒し込みのタイミングでフロントフォークのストローク（何%か）や減速Gがどう変動しているか（ブレーキを残せているか、リリースが早すぎるか）をリンクさせて解析すること。
"""
